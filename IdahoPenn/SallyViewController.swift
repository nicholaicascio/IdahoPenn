...
class SallyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        do{
            let audiopath = Bundle.main.path(forResource: "sallyport_caption", ofType: "wav")
    ...
    @IBAction func imgTapped1(_ sender: UITapGestureRecognizer) {
    ...
    @IBAction func imgTapped2(_ sender: UITapGestureRecognizer) {
    let imageView = sender.view as! UIImageView
    let newImageView = UIImageView(image: imageView.image)
    newImageView.frame = self.view.frame
    newImageView.backgroundColor = .black
    ...
    @IBAction func imgTapped3(_ sender: UITapGestureRecognizer) {
    let imageView = sender.view as! UIImageView
    let newImageView = UIImageView(image: imageView.image)
    newImageView.frame = self.view.frame
    newImageView.backgroundColor = .black
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.destination is AudioTabViewController{
          // now set a var that points to that new viewcontroller so you can call the method correctly
            ...
            nextController.setResultLabel(index: 5)
    }
    }
